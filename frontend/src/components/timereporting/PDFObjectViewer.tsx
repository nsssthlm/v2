...
/**
 * En PDF-visare som använder PDFObject-biblioteket för att visa PDF-dokument 
 * direkt i dialogrutan. PDFObject är en robust lösning som fungerar i de flesta 
 * moderna webbläsare.
 */
const PDFObjectViewer: React.FC<PDFObjectViewerProps> = ({
  pdfUrl,
  filename,
  onDownload,
  onOpenInNewTab
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Vänta en kort stund och dölj sedan laddningsindikatorn
      const timer = setTimeout(() => {
        setLoading(false);
        
        // Om PDFObject rapporterar att det inte kunde visas
        if (!success) {
          setPdfError(true);
        }
      }, 1500);
