...
interface BlobPDFViewerProps {
  pdfUrl: string;  // Detta måste vara en blob: URL
  filename: string;
}

/**
 * En specialiserad PDF-visare som bara fungerar med blob: URL:er
 * Löser säkerhetsproblem med webbläsarens sandbox-begränsningar
 */
const BlobPDFViewer: React.FC<BlobPDFViewerProps> = ({ 
  ...
  // Säkerställ att det är en blob URL
  if (!pdfUrl.startsWith('blob:')) {
    console.error('BlobPDFViewer måste användas med en blob: URL!');
  }
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ flexGrow: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        {loadError && (
          ...
          >
            ...
            <Typography sx={{ mb: 4, color: 'text.secondary' }}>
              PDF-filen kunde inte visas direkt i dialogrutan.
              <br />
        ...
        {/* PDF-visning med iframe (fungerar med blob: URLs) */}
        {!loadError && (
          <iframe
            src={pdfUrl}
            style={{ 
