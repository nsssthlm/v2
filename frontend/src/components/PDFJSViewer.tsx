...
const PDFJSViewer: React.FC<PDFJSViewerProps> = ({
  ...
  const processedUrl = React.useMemo(() => {
    ...
    // Ersätt alla lokala URL:er (t.ex. 0.0.0.0:8001 eller localhost:8001) med API_BASE
    if (pdfUrl.includes("0.0.0.0:8001") || pdfUrl.includes("localhost:8001")) {
      finalUrl = pdfUrl
        .replace("http://0.0.0.0:8001", API_BASE)
        .replace("http://localhost:8001", API_BASE);

      console.log("Uppdaterad URL med API_BASE:", finalUrl);
    }
    ...
    // Strategi 2: Om URL:en kommer från API-endpoint, extrahera sökvägen därifrån
      ...
      const apiPattern =
        /\/api\/files\/web\/.*?\/data\/project_files\/(\d{4}\/\d{2}\/\d{2}\/[^?&]+\.pdf)/;
      const apiMatch = finalUrl.match(apiPattern);

      if (apiMatch && apiMatch[1]) {
        finalUrl = `${API_BASE}/pdf/${apiMatch[1]}`;
      ...
      // Om inget matchande mönster men vi har ett filnamn, använd pdf-finder med filnamnet
        ...
        finalUrl = `${API_BASE}/pdf-finder/?filename=${fileName}&stream=true`;
    ...
    switch (attemptNum) {
      ...
      case 4:
        ...
        if (baseNameMatch) {
          ...
          const nameWithExt = baseName.endsWith(".pdf")
            ? baseName
            : `${baseName}.pdf`;
          return `${API_BASE}/pdf-finder/?filename=${nameWithExt}&stream=true`;
        }
        break;
    }
    ...
    const fetchPdf = async (url: string, alternativeAttempt = false) => {
      ...
      try {
        if (attemptCount === 1) {
          ...
          console.log(
            `Försök ${attemptCount}/${maxAttempts}: ${alternativeAttempt ? "Alternativ" : "Direkt"} URL:`,
            url,
          );
        }
        ...
        if (!fetchResponse.ok) {
          console.warn(
            `Servern svarade med ${fetchResponse.status} (${fetchResponse.statusText}) för URL: ${url}`,
          );
        ...
        if (!contentType?.includes("application/pdf")) {
          console.warn(
            "Servern returnerade oväntad Content-Type:",
            contentType,
          );
        }
        ...
        if (isPdfArrayBuffer(buffer)) {
          console.log(
            `PDF hämtad framgångsrikt, storlek: ${buffer.byteLength} bytes från URL: ${url}`,
          );
          if (isMounted) {
            setPdfBytes(buffer);
          }
        ...
        console.error(
          `Fel vid hämtning av PDF (försök ${attemptCount}/${maxAttempts}):`,
          err,
        );

        if (attemptCount < maxAttempts) {
          // Använd vår smarta generering av alternativa URL:er baserat på vilket försök vi är på
          const alternativeUrl = createAlternativeUrl(
            processedUrl,
            attemptCount,
          );
          ...
          if (alternativeUrl !== url) {
            console.log(
              `Provar alternativ URL (${attemptCount}):`,
              alternativeUrl,
            );
          ...
          if (isMounted) {
            setError(
              `Kunde inte ladda PDF: Försökte ${maxAttempts} gånger med olika URL-format men misslyckades. Försök öppna i ny flik.`,
            );
            setIsLoading(false);
          }
        }
  ...
  useEffect(() => {
    ...
    const renderPdf = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          const errorMessage =
            err instanceof Error ? err.message : "Okänt fel vid PDF-rendering";
    ...
    const renderPage = async (
      ...
      try {
        ...
        if (isMounted) {
          setError(
            err instanceof Error ? err.message : "Kunde inte rendera PDF-sidan",
          );
        }
      }
    };
